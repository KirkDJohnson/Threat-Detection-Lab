...
<h2>Description</h2> 
In the CTF/walkthrough scenario, a company using a business management platform wanted to determine if they had been compromised. Taking on the role of a security analyst, I was given a log and packet capture file. I began my investigation with the log file and, discovering it was a large JSON-formatted file, used the tool jq to more easily view it. I identified the "signature" field within the alert category and filtered for alerts by IP addresses, sorting and counting the number of occurrences. This revealed a large volume of web traffic from a user agent "python-requests" and possible staging of CVE-2022-25237. I then conducted threat intelligence on the CVE and discovered that the vulnerability was related to the business management platform, Bonita, which the company was using. This vulnerability allowed the attacker to append a string to the end of a URL, giving them access to privileged APIs and resulting in remote code execution. Knowing this, I pivoted to Wireshark and filtered for the IP with the highest "python-requests" activity. This revealed that the attacker had engaged in a credential stuffing attack against /bonita/loginservice and successfully logged in. Following the HTTP stream in Wireshark, I observed that after the attacker gained access to the web server, they leveraged the vulnerability within Bonita, appended a string to the URL, and subsequently gained remote code execution. Their next HTTP request used the command prompt with the command "whoami" to see the user they had access to. The other IP address immediately logged in with the same credentials as the first IP, signifying it was part of the same attack. This IP address also obtained remote code execution by leveraging the same vulnerability and used three commands: "cat /etc/passwd," a wget to pastes[.]io/raw/****et8, and then ran the downloaded script in bash. I then investigated the wget URL using Browserling and discovered it contained a curl command to another pastes.io site, where it downloaded an SSH key and moved it to .../.ssh/authorized_keys, followed by "sudo service ssh restart." This created a persistence mechanism for the attacker to SSH into the web server with ease. Following these packets in Wireshark, the malicious IP was seen establishing an SSH session, where data could have been exfiltrated.
